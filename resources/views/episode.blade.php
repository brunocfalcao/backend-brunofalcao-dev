	...
	<x-slot:main_content>
		<div class="w-full grid 2xl:grid-cols-3 gap-6">
			<div class="order-2 2xl:order-1 2xl:col-span-1">
				...
				<div class="space-y-4 mt-6">
					@foreach($course_chapters as $chapter_i)
						<div class="flex gap-2">
							...
							<div class="space-y-4 w-full @if ($loop->index >= 1) pt-[.45rem] @endif">
								<p class="font-semibold text-black text-large">{{ $chapter_i->name }}</p>
								@foreach($chapter_i->episodes as $currentEpisode)
								<div class="py-2 px-0 w-full rounded-xl bg-white flex items-center gap-2 {{--opacity-50 select-none--}}">
									@if($currentEpisode->is_seen == true)
									@svg('heroicon-s-check-circle', 'h-5 w-5 flex-shrink-0 text-progress-bar-foreground')
									@else
									...
									<p class="font-semibold text-sm flex-shrink-0 text-gray-400 ml-auto">{{$currentEpisode->duration_for_humans}}</p>
								</div>
								@endforeach
							</div>
					</div>
		...
		<div class="order-1 2xl:order-2 2xl:col-span-2">
			...
			<div class="py-6 border-b border-gray-200">
				<p class="text-gray-600 font-medium text-sm">{{$chapter->name}} • (1/4)</p>
